...
class Superset(BaseSupersetView):
    ...
    @log_this
    @has_access_api
    @expose('/update_table/<datasource_id>/<table_name>/', methods=['POST', 'GET'])
    def update_table(self, datasource_id, table_name):
        """Runs arbitrary sql and returns and json"""
        table = table_name or "Employees"
        col= request.values.get('col') or "title"
        val = request.values.get('val') or "'king'"
        key = request.values.get('key') or "EmployeeId" 
        keyValue = request.values.get('keyValue') or 100        
        #sql = "UPDATE Employees SET title = 'King' WHERE EmployeeId =100"
        sql =unicode("UPDATE "+table+" SET "+col+" = "+val+" where "+key+"="+keyValue)   
        #return sql
        database_id = datasource_id or 2
        schema = request.values.get('schema') or None
        ...
        session.commit()
        mydb.get_sqla_engine().execute(text(sql).execution_options(autocommit=True))
        return "success"

    @log_this
    @has_access_api
    @expose('/selectData/<datasource_id>/<table_name>/', methods=['POST', 'GET'])
    def selectData(self, datasource_id, table_name):
        """Runs arbitrary sql and returns and json"""
        async = False
        selectCol = request.values.get('selectCol') or "*"
        tableName = table_name or "Employees"
        whereClause = request.values.get('whereClause') or ""
        if whereClause != "":
            whereClause = "WHERE "+ whereClause
        sql = unicode("SELECT "+selectCol+ " FROM "+tableName+" "+whereClause)
        database_id = datasource_id
        schema = request.values.get('schema') or None
        template_params = json.loads(
            request.values.get('templateParams') or '{}')
        ...
        logging.info(sql)
        cursor.execute(sql,
                       **mydb.db_engine_spec.cursor_execute_kwargs)
        data = mydb.db_engine_spec.fetch_data(cursor, int(app.config.get('SQL_MAX_ROW', None)))
        cdf = sql_lab.convert_results_to_df(cursor.description, data)
        payload = json.dumps(
                cdf.data if cdf.data else [], default=utils.pessimistic_json_iso_dttm_ser)
        return payload
