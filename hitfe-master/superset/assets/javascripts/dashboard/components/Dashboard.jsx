...
class Dashboard extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    var that = this;
    $.ajax({
          url: '/superset/selectData/2/Key_Ref/',
          method: 'GET',              
          data: {
              selectCol : "id, Name"
          },                                        
          success: function(data) {
              ...
              var _dataObj = {};
              for(var i in data){
                _dataObj[data[i].id] = data[i].Name;
              }
              breadcrumb(_dataObj, that.props.userRole, that.props.dashboardId);
          }
    });

    function breadcrumb(navObj, userRole, dashboardId){
      var brdCrmbHtml = "<ul>";

      if(userRole == "head_of_operations"){

        brdCrmbHtml = brdCrmbHtml + "<li class='breadcrumb_hd'><a href='/superset/dashboard/company/?comp=" + getParameterByName("comp") + "'>MY DASHBOARD</a></li>"; 

        if(dashboardId == "factory" || dashboardId == "productionline" || dashboardId == "asset"){

          brdCrmbHtml = brdCrmbHtml + "<li class='breadcrumb_hd'><a href='/superset/dashboard/factory/?comp=" + getParameterByName("comp") + "&fac=" + getParameterByName("fac") + "'>"+ navObj[getParameterByName("fac")] +"</a></li>"; 

        }

        if(dashboardId == "productionline" || dashboardId == "asset"){

          brdCrmbHtml = brdCrmbHtml + "<li class='breadcrumb_hd'><a href='/superset/dashboard/productionline/?comp=" + getParameterByName("comp") + "&fac=" + getParameterByName("fac") +  "&prod=" + getParameterByName("prod") + "'>"+ navObj[getParameterByName("prod")] +"</a></li>"; 

        }

        if(dashboardId == "asset"){

          brdCrmbHtml = brdCrmbHtml + "<li class='breadcrumb_hd'><a href='/superset/dashboard/asset/?comp=" + getParameterByName("comp") + "&fac=" + getParameterByName("fac") +  "&prod=" + getParameterByName("prod") +  "&ast=" + getParameterByName("ast") + "'>"+ navObj[getParameterByName("ast")] +"</a></li>"; 

        }

      }else if(userRole == "shift_supervisor"){

        brdCrmbHtml = brdCrmbHtml + "<li class='breadcrumb_hd'><a href='/superset/dashboard/factory/?fac=" + getParameterByName("fac") + "'>MY DASHBOARD</a></li>"; 

        if(dashboardId == "productionline" || dashboardId == "asset"){

          brdCrmbHtml = brdCrmbHtml + "<li class='breadcrumb_hd'><a href='/superset/dashboard/productionline/?fac=" + getParameterByName("fac") +  "&prod=" + getParameterByName("prod") + "'>"+ navObj[getParameterByName("prod")] +"</a></li>"; 

        }

        if(dashboardId == "asset"){

          brdCrmbHtml = brdCrmbHtml + "<li class='breadcrumb_hd'><a href='/superset/dashboard/asset/?fac=" + getParameterByName("fac") +  "&prod=" + getParameterByName("prod") +  "&ast=" + getParameterByName("ast") + "'>"+ navObj[getParameterByName("ast")] +"</a></li>"; 

        }
      }else if(userRole == "line_manager"){

        brdCrmbHtml = brdCrmbHtml + "<li class='breadcrumb_hd'><a href='/superset/dashboard/productionline/?prod=" + getParameterByName("prod") + "'>MY DASHBOARD</a></li>"; 

        if(dashboardId == "asset"){

          brdCrmbHtml = brdCrmbHtml + "<li class='breadcrumb_hd'><a href='/superset/dashboard/asset/?prod=" + getParameterByName("prod") +  "&ast=" + getParameterByName("ast") + "'>"+ navObj[getParameterByName("ast")] +"</a></li>"; 

        }
      }
      brdCrmbHtml = brdCrmbHtml + "</ul>";

      document.getElementById('brandHeader').insertAdjacentHTML('beforeend', brdCrmbHtml);
    }

    var colName = "Asset_ID";
    var colValue = "'ASST01'";
    var alertHref = "";
    if(this.props.userRole == "head_of_operations")
    {
      colName = "Company_ID";
      colValue = "'"+getParameterByName('comp')+"'";
      alertHref = "/superset/dashboard/companyalerts/?comp="+ getParameterByName('comp');
    }
    else if(this.props.userRole == "shift_supervisor")
    {
      colName = "Factory_ID";
      colValue = "'"+getParameterByName('fac')+"'";
      alertHref = "/superset/dashboard/factoryalerts/?fac="+ getParameterByName('fac');
    }
    else if(this.props.userRole == "line_manager")
    {
      colName = "Line_ID";
      colValue = "'"+getParameterByName('prod')+"'";
      alertHref = "/superset/dashboard/productionlinealerts/?prod="+ getParameterByName('prod');
    }else{
      alertHref = "/superset/dashboard/companyalerts/?comp="+ getParameterByName('comp')
    }

    $("#alertIconCount").find('a').attr('href',alertHref);

    $("#alertContent").click(function(e){
      if($(e.target).is('.closeAlert')){
          e.preventDefault();
          return false;
      }
      window.open(alertHref,'_blank');
    });

    var currentDate = new Date();
    currentDate = currentDate.toISOString().split('.')[0];
    currentDate = currentDate.split('T')[0];
    currentDate = currentDate + "T00:00:00";
    var firstAlertCnt = 0, firstAlert = true;
    function fetchAlertData(){ 
        $.ajax({
                  url: '/superset/selectData/2/Alert_Details/',
                  method: 'GET',              
                  data: {
                      selectCol : "count(*) as cnt",
                      whereClause : colName+"="+colValue+" and Alert_Status = 'Critical' and Date_Time >= '"+ currentDate +"' and Completed_Time is null and Alert_Priority = 'High'"
                    },                                        
                  success: function(resp) {
                    resp = JSON.parse(resp);
                    var item = parseInt(resp[0]["cnt"]);

                    var _firstAlertCnt = 0;

                    if(firstAlertCnt > 0){
                      _firstAlertCnt = firstAlertCnt;
                    }else{
                      if(firstAlertCnt == 0){
                        _firstAlertCnt = 0;
                      }else{
                    ...
                    document.getElementById("alertCountVal").innerHTML = item;
                    document.getElementById("alertCountVal").style.display = "block";
                  
                    if(item > _firstAlertCnt && !firstAlert)
                    {
                      document.getElementById("alertContent").style.display = "none";
                      document.getElementById("alertSound").play();
                      document.getElementById("notifMsg").innerHTML = "You have a new alert!";
    ...
    setInterval(fetchAlertData,15000);
    ...
    return (
      <div id="dashboard-container">
        <div id="dashboard-header">
          ...
          <div id="brandHeader" className="brandHeader">

            
            <audio id="alertSound" src="/static/assets/images/alertSound.mp3" preload="auto"></audio>

        <span className = "alertContent" id = "alertContent">
          <label id = "notifMsg" className="notifMsg">
            {/* <i className="fa fa-exclamation-triangle" styles ="color:#cc0000; font-size:20px;"></i> */}
            <i className="fa fa-exclamation-triangle" style={{ fontSize: '20px', color: '#cc0000' }}></i>
          </label> 
          <label className = "closeAlert">X</label></span>
            </div>
           
        

         
          { this.props.editMode || this.props.userRole == 'Admin'? 
            <Header
