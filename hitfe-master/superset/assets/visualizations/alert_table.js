...
function alertTableViz(slice, payload) {
    ...
    if(window.location.pathname.indexOf("companyalerts") != -1){
        isSummary = true;
    }
    ...
    class DataTableComponent extends React.Component {
        ...
        showSearchTool(el) { 
          ...
          changeTimeFormat(cell, row){
           ...
           timeTakenFormat(cell, row){
            ...
            assignedTo(cell, row){
                var output = row.assignTo;
                if(row.assignTo == "NULL" || row.assignTo == null ){
                    output = ''; 
                }
                return output; 
            }

            locationMerge(cell, row){
                return row.location + " > " + row.assetName;
            }
       ...
       render() {
          ...
          return ( 
            <Panel id="tableGridPanel" className = "tableGridPanel">
                  ...
                  <input type="hidden" value={this.state.activeTabKey} />
                    {isSummary ? (
                        <BootstrapTable ref='alertsTable' containerClass="alertsTable" data={filteredData} striped hover bordered={ false } search={isSearchEnabled} multiColumnSearch options={ this.options }> 
                            ...
                            <TableHeaderColumn dataSort dataField='assetName'>Asset</TableHeaderColumn>
                            <TableHeaderColumn dataSort dataField='location' >Location</TableHeaderColumn>
                            <TableHeaderColumn dataSort dataField='assignTo' dataFormat={ this.assignedTo}>Assigned To</TableHeaderColumn>
                        </BootstrapTable>
                    ) : (
                        <BootstrapTable ref='alertsTable' containerClass="alertsTable" data={filteredData} striped hover bordered={ false } search={isSearchEnabled} multiColumnSearch options={ this.options }> 
                            <TableHeaderColumn isKey dataSort dataField='id' width='70px' dataFormat={ this.alertDetail }>ID</TableHeaderColumn>
